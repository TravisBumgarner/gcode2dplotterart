...
class Plotter:
  ...
  def add_layer(self, title: str, preview_only:bool=False) -> Layer:
    ...
    self.layers[title] = Layer(units=self.units, plotter_type=self.plotter_type, x_min= self.x_min, x_max=self.x_max, y_min=self.y_min, y_max=self.y_max, feed_rate=self.feed_rate, handle_out_of_bounds=self.handle_out_of_bounds, preview_only=preview_only)

  def get_min_and_max_points(self) -> Dict[Literal["x_min", "y_min", "x_max", "y_max"], float]:
    """
  ...
  def add_border_layer(self) -> None:
    ...
    self.layers['border'].add_rectangle(points['x_min'], points['y_min'], points['x_max'], points['y_max'])
  ...
  def add_preview_layer(self) -> None:
    ...
    self.layers['preview'] \
      .add_rectangle(points['x_min'], points['y_min'], points['x_max'], points['y_max']) \
      .add_line(points['x_min'], points['y_min'], points['x_max'], points['y_max']) \
      .add_line(points['x_min'], points['y_max'], points['x_max'], points['y_min'])
