...
- `Reset Zero` - This will set the print head's current position to `X = 0`, `Y = 0`, and optionally `Z = 0`. All future movements will be made relative to this position. 
...
These instructions are specific to 2D plotters. Check out the [G-Code Docs](./gcode) for other instructions that can be bade.
...
Once you've initially performed [getting your plotting device's dimensions](#get-your-plotting-devicess-dimensions), this step should be performed every time you power on your device.
...
4. If you have a 3D printer, attach your drawing instrument to the print head. Move your print head in the negative Z Direction (`Z-`) until your pen is not touching the drawing surface. Note - Your 3D printer might have its Z Axis flipped. You instead might need to move in the positive Z Direction (`Z+`) to move the drawing instrument away from the drawing surface.
5. Switch to the Toolbox tab and Click `Reset Zero`. Now we have our starting point for all plotting. 
6. Now move the print head around in the X, Y, and optionally Z directions.
