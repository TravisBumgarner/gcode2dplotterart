...
```python
__init__(
self, title: str, x_min: float, x_max: float, y_min: float, y_max: float, feed_rate: float, handle_out_of_bounds: Union[Literal['Warning'], Literal['Error'], Literal['Partial']] = 'Partial', output_directory: str = './output', include_comments: bool = True, return_home_before_plotting: bool = True )
 ->  None
```
...
```python
add_layer(
self, title: str, color: Optional[str] = '#000000', line_width: float = 2.0, preview_only: bool = False )
